...
import { RadioGroup } from '@alfalab/core-components/radio-group/modern';
...
const ASSET_OPTIONS = getKeys(Asset).map((key) => ({
    key: Asset[key],
    content: formatAssetName(Asset[key]),
}));
...
const Demo: FC = () => {
    ...
    const handlePackageChange = (value: string) => {
        setPackages(value as IconPackageName);
    };
    ...
    const renderHeader = () => {
        return (
            <div key='header'>
                ...
                {['icons', 'logotype'].includes(asset) ? <RadioGroup
                    onChange={(_, payload) => handlePackageChange(payload.value)}
                ...
                >
                    {ASSET_TO_PACKAGE_NAME[asset].map((assetItem) => (
                        <Tag shape='rectangular' value={assetItem.value} key={assetItem.value}>
                            {assetItem.label}
                        </Tag>
                    ))}
                </RadioGroup> : null}
